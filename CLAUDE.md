...
The SecureLoop design system is authoritative across **four** sources. Each has a specific role. If any two conflict, use this precedence:

1. **§6–§8 of this file (v1 tokens, locked patterns, exclusions)** — WINS for anything in scope. v1 values here are FROZEN.
2. **`docs/11_design_system_v2_draft.md`** — WINS for everything NOT in §6–§8: v2 additive scope (elevation, motion, density, data-viz palettes, micro-interactions, empty/loading/error states, table patterns beyond v1, keyboard/accessibility extensions, typography rhythm, icon system, chrome refinements, print styles, responsive rules, brand moments). v2 is **additive only** — it must never change meaning or value of any v1 token.
3. **`packages/design-system/` and `packages/ui/` source code** — IMPLEMENTATION of the above. When code exists, it is the runtime truth. When code diverges from v1 or v2 spec, the SPEC wins — raise a remediation task, do not modify the spec to match the code.
4. **`wireframes/<PERSONA>/*.html`** — WINS for layout and visual composition of specific screens. Pixel-match the wireframe.

### Build rule for `packages/design-system/` and `packages/ui/`

When building or modifying either package:

- Source v1 tokens **from §6 of this file** — copy values exactly. Do not read them from the `.md` doc or wireframes.
- Source v1 patterns **from §7 of this file** — enforce every rule.
- Source v2 tokens and patterns **from `docs/11_design_system_v2_draft.md`** — implement faithfully.
- Use wireframes ONLY to confirm visual composition of final screens, not to derive component APIs.
- If both v1 and v2 are silent on a behaviour → STOP and ask Mahesh. Do not guess.

### What lives where

| Source | Role |
|---|---|
| `CLAUDE.md` §6 (v1 tokens) | Exact hex/px values — runtime constants, FROZEN |
| `CLAUDE.md` §7 (v1 locked patterns) | Interaction rules that must never break |
| `CLAUDE.md` §8 (exclusions) | Things never to do |
| `docs/11_design_system_v2_draft.md` | v2 additive scope — elevation, motion, density, dataviz, MI, states, table patterns, a11y, typography rhythm, icons, print, responsive, brand |
| `packages/design-system/src/tokens/*` | v1 values reproduced as TS/CSS for import |
| `packages/design-system/src/themes/*` | v1 palettes + v2 elevation/dataviz as theme vars |
| `packages/design-system/src/{elevation,motion,dataviz}/*` | v2 additive token modules |
| `packages/ui/src/components/*` | React components built on design-system, matching v1 §7 + v2 spec |
...
Hi-fi HTML wireframes for frozen personas (CISO, CTRL, TSTR, SENG v1) live in `/wireframes/<PERSONA>/`.

**When building a screen, always:**
1. Read the wireframe spec in `docs/09_wireframe_specs/<PART>.md`.
2. Open the corresponding HTML wireframe if it exists.
3. Match the wireframe pixel-accurately. Do not improvise layout.
...
10. Run `pnpm lint && pnpm typecheck && pnpm test` before declaring done.
